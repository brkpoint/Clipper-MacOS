...
extension AXUIElement {
    func getValue(_ attribute: NSAccessibility.Attribute) -> AnyObject? {
        ...
        guard result == .success else { return nil } //print("ERR: \(result.rawValue)")
        return value
    }
    ...
    private func m_setValue(_ attribute: NSAccessibility.Attribute, _ value: AnyObject) {
        ...
        guard error == .success else { return } //print("ERR: AXSetValue - err_code: \(error.rawValue)")
    }
...
extension AXValue {
    func convertTo<T>() -> T? {
        ...
        if !success { return nil } //print("ERR: AXValue error")
        return value
    }
